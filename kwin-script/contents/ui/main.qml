...
Item {
    ...
    function activeLayout() {
        return settings.layouts[activeLayoutIndex()];
    }

    function zoneRect(zone, area) {
        const padding = Math.max(0, settings.outer_padding || 0) + Math.max(0, activeLayout().padding || 0);
    ...
    function snapActiveWindowToZone(index) {
        moveClientToZone(Workspace.activeWindow, index);
    }
    ...
    function moveClientToZone(client, index) {
        ...
        if (!layout || index < 0 || index >= layout.zones.length)
            return false;
        refreshArea(client);
        const rect = zoneRect(layout.zones[index], activeArea);
        if (client.setMaximize)
            client.setMaximize(false, false);
        client.frameGeometry = rect;
        client.fanzyZone = index;
        client.fanzyLayout = activeLayoutIndex();
    ...
    function nearestZoneIndex(client) {
        ...
        for (let i = 0; i < layout.zones.length; i++) {
            const rect = zoneRect(layout.zones[i], activeArea);
    ...
    function snapClientToClosestZone(client) {
        const index = nearestZoneIndex(client);
        if (index >= 0)
            return moveClientToZone(client, index);
        return false;
    }
    ...
    function cycleActiveWindow(delta) {
        ...
        if (isSkippedWindow(client))
            return;
        const layout = activeLayout();
        let current = client.fanzyLayout === activeLayoutIndex() ? client.fanzyZone : nearestZoneIndex(client);
        if (current < 0)
            current = 0;
    ...
    function snapAllWindows() {
        ...
        for (let i = 0; i < all.length; i++)
            snapClientToClosestZone(all[i]);
    }

    function updateHighlightedZone() {
        if (!movingWindow)
            return;
        refreshArea(movingWindow);
        highlightedZone = -1;
        ...
        if (settings.enable_zone_selector && cursor.y >= activeArea.y && cursor.y <= activeArea.y + selectorHeight) {
            ...
            for (let i = 0; i < selectedLayout.zones.length; i++) {
                ...
                if (contains(mini, cursor)) {
                    highlightedZone = i;
                    return;
                }
            }
            if (localX >= 0)
                highlightedZone = nearestZoneIndex(movingWindow);
            return;
        }
    ...
    function connectWindow(client) {
        if (!client || connectedWindows[windowId(client)])
            return;
        connectedWindows[windowId(client)] = true;

        if (client.onInteractiveMoveResizeStarted) {
            client.onInteractiveMoveResizeStarted.connect(function() {
                if (isSkippedWindow(client) || !client.move)
                    return;
                moving = true;
                moved = false;
                movingWindow = client;
                highlightedZone = -1;
                saveOriginalGeometry(client);
                refreshArea(client);
                overlay.showOverlay();
            });
            client.onInteractiveMoveResizeStepped.connect(function() {
                if (moving && movingWindow === client)
                    moved = true;
            });
            client.onInteractiveMoveResizeFinished.connect(function() {
                if (moving && movingWindow === client && moved && (modifiersSatisfied() || overlayForced) && highlightedZone >= 0)
    ...
    function desktopHasWindows(desktop) {
        ...
        for (let i = 0; i < all.length; i++) {
            const client = all[i];
            if (clientOnDesktop(client, desktop) && !client.skipPager && !client.onAllDesktops)
                return true;
        }
        return false;
    }
    ...
    function createDesktopForWindow(client) {
        ...
        const insertAt = settings.macsimize_move_to_last_desktop
            ...
            : Math.max(1, desktopIndex(Workspace.currentDesktop) + 1);
    ...
    function restoreMacsimizedWindow(client) {
        ...
        const idx = managedDesktopIndex(desktop);
        if (idx >= 0)
    ...
    function handleMinimizedChanged(client) {
        const saved = savedWindows[windowId(client)];
    ...
    function handleNewWindow(client) {
        ...
        if (client.transient && client.transientFor) {
            const parentSaved = savedWindows[windowId(client.transientFor)];
        ...
        if (settings.macsimize_exclusive_desktops && managedDesktopIndex(Workspace.currentDesktop) >= 0 &&
        ...
        if (settings.auto_snap_new_windows)
            snapClientToClosestZone(client);
        ensureTrailingEmptyDesktop(client);
    }
    ...
    Component.onCompleted: {
        ...
        const all = windows();
        for (let i = 0; i < all.length; i++)
            handleNewWindow(all[i]);
        if (Workspace.windowAdded)
            Workspace.windowAdded.connect(handleNewWindow);
        if (Workspace.currentDesktopChanged)
            Workspace.currentDesktopChanged.connect(pruneEmptyDesktops);
        ensureTrailingEmptyDesktop(null);
    ...
    PlasmaCore.Dialog {
        ...
        Item {
            id: overlayContent
            anchors.fill: parent
            visible: settings.snap_mode === "auto" || modifiersSatisfied() || overlayForced

            Repeater {
                model: activeLayout() ? activeLayout().zones : []
                delegate: Rectangle {
                    property var rect: zoneRect(modelData, activeArea)
                    x: rect.x
                    y: rect.y
                    width: rect.width
                    height: rect.height
                ...
                Repeater {
                    ...
                    delegate: Item {
                        ...
                        PlasmaComponents.Label {
                            ...
                            text: modelData.name
                            color: index === activeLayoutIndex() ? "white" : "#d6d6d6"
                            font.pixelSize: 11
                        }
                        ...
                        Repeater {
                            ...
                            delegate: Rectangle {
                                ...
                                radius: 2
                                color: Qt.rgba(settings.highlight_color.red, settings.highlight_color.green, settings.highlight_color.blue, index === highlightedZone && layoutDelegate.layoutIndex === activeLayoutIndex() ? 0.70 : 0.28)
    ...
    ShortcutHandler {
        ...
        onActivated: cycleActiveWindow(1)
    }
    ...
    ShortcutHandler {
        ...
        onActivated: snapClientToClosestZone(Workspace.activeWindow)
    }
    ...
    Repeater {
        ...
        delegate: ShortcutHandler {
            ...
            onActivated: snapActiveWindowToZone(modelData - 1)
        }
    }
    ...
    Repeater {
        ...
        delegate: ShortcutHandler {
            ...
            onActivated: setActiveLayout(modelData - 1)
        }
    }
}
